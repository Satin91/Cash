...
func addObject(text: String, image: UIImage?,sum: Double?){
    var summ: Double = 0
    if sum != nil {
        summ = sum!
    }
    
    let object2 = MonetaryEntity(name: text, sum: summ, userDescription: nil, date: Date(), image: image?.pngData(), accountType:  .operationExpence, userPerent: 0)
